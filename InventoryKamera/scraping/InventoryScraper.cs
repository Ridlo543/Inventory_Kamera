...
namespace InventoryKamera
{
    ...
    internal class InventoryScraper
    {
        ...
        internal (List<Rectangle> rectangles, int cols, int rows) ProcessScreenshot(Bitmap screenshot, double weight = 0)
        {
            ...
            int blobMinHeight = Math.Max(1, (int)(iconMinHeight * (1 - weight)));
            int blobMaxHeight = Math.Max(blobMinHeight, (int)(iconMaxHeight * (1 + weight)));
            int blobMinWidth = Math.Max(1, (int)(iconMinWidth * (1 - weight)));
            int blobMaxWidth = Math.Max(blobMinWidth, (int)(iconMaxWidth * (1 + weight)));
            ...
            })
            {
                ...
                double colTolerance = Math.Max(icon.Width * 0.45, 55 / base_aspect_width * screenshot.Width);
                double rowTolerance = Math.Max(icon.Height * 0.45, 70 / base_aspect_height * screenshot.Height);
                ...
                foreach (var item in rectangles)
                {
                    ...
                    foreach (var x in colCoords)
                    {
                        ...
                        if (x - colTolerance <= xC && xC <= x + colTolerance)
                        {
                            addX = false;
                            break;
                        }
                    ...
                    foreach (var y in rowCoords)
                    {
                        ...
                        if (y - rowTolerance <= yC && yC <= y + rowTolerance)
                        {
                            addY = false;
                            break;
                        }
                ...
                try
                {
                    ...
                    do
                    {
                        ...
                        if (itemCount != itemPerPage && !acceptLess)
                        {
                            ...
                            double step = Math.Abs(delta) >= 16 ? 0.12 : (Math.Abs(delta) >= 8 ? 0.07 : 0.04);
                            weight += delta > 0 ? step : -step;
